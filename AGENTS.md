...
- Before updating, rebuilding, starting, stopping, or redeploying a Portainer Stack, inspect and record the current Stack file, the complete set of Stack environment-variable names and values, the Stack webhook UUID, registry associations, and the target environment. Treat these items as required deployment state, not optional update fields.
- Every Portainer Stack update or rebuild must preserve the existing environment variables and webhook UUID unless the user explicitly requests a change. Never submit a partial `Env` list, never replace an existing webhook with an empty value, and never reuse placeholder values such as `[REDACTED]` as real configuration.
- **Known Portainer `StackUpdate` trap:** omitting the `Env` field does not preserve the existing Stack environment-variable metadata. It clears the Stack's `Env` list, even when the update is intended only to change the Compose file, image, webhook, or another unrelated field. Every `StackUpdate` call must therefore include the complete current `Env` array with all real values. Never rely on omission semantics for ENV preservation.
- If the Portainer tool redacts environment-variable values, do not update or rebuild the Stack until the real values are available through an approved secure source or the user supplies them. Omitting a required variable or guessing its value is not an acceptable workaround.
- When adding a required environment variable to a Compose file, add it to the Portainer Stack environment-variable configuration in the same operation. Before deployment, compare all `${...}` references in the Compose file with the Stack's configured variable names and stop if any required variable is missing.
- Preserve or explicitly create the Stack webhook during Stack updates. Afterward, verify that Portainer reports a non-empty webhook UUID and that the corresponding GitHub Actions workflow uses that exact UUID.
- Deploy Compose workloads through Portainer. Do not run `docker compose up` manually from a Portainer version directory such as `compose/<stack-id>/v<N>` without explicitly setting the canonical project name; otherwise Compose may label the project as `v<N>` and Portainer will report the Stack as inactive. The container label `com.docker.compose.project` must equal the Portainer Stack name.
- Do not treat a successful webhook HTTP response as proof of a successful deployment. After every Stack mutation or webhook invocation, re-read the Stack and verify that it is active, its expected containers carry the correct Compose project label, the containers are running and healthy, the expected image was deployed, and a representative application endpoint works.
- If a Stack is inactive while a same-named container is running under a different Compose project label, preserve all ENV and webhook state first, then remove the orphaned container, redeploy through Portainer with the canonical project name, and complete the post-deployment verification above.
