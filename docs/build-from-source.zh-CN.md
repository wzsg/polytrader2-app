...
| `P2_REMOTE_ACCESS_URL`                                       | 启用远程访问，并设置桌面客户端主动连接的 `ws://` 或 `wss://` Tunnel 地址。                            |
| `P2_REMOTE_ACCESS_TOKEN`                                     | 桌面客户端在 Tunnel 握手时发送的认证令牌，请使用高熵密钥。                                            |
| `P2_REMOTE_ACCESS_DEVICE_ID`                                 | 稳定的桌面设备标识，用于请求路由以及断线重连后的身份保持。                                            |
